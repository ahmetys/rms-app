...
function Customers() {
  ...
  useEffect(() => {
    const getAllCustomers = async () => {
      const { data } = await axios.get(`${API_URL}/api/customers/getAllCustomers`, { withCredentials: true });
      setCustomers([...data]);
    };
    getAllCustomers();
  }, []);
  const deleteCustomer = async (customerId) => {
    const { data } = await axios.delete(`${API_URL}/api/customers/deleteCustomer/${customerId}`, { withCredentials: true });
    if (data.succeeded) {
    ...
    <>
      <section className="border drop-shadow-xl bg-white rounded mb-5">
        <div className="p-5 bg-gray-50 border-b grid grid-cols-5 justify-between items-center relative">
          <h1 className="col-span-4 text-4xl font-semibold">Kunden</h1>
                ...
                <table className="table-auto min-w-full overflow-auto overscroll-contain">
                  <thead className="border-b font-bold">
                    <tr>
                      ...
                      <td className="py-3 px-2">Telefonnummer</td>
                      <td className="py-3 px-2">E-Mail</td>
                      <td className="py-3 px-2">Addresse</td>
